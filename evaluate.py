    ...
    with open(logfile, "w") as f:
        ...
        for i in range(len(frames)):
            ...
            try:
                X = keyframe.X_canon
                C = keyframe.C
                X_flatten = X.cpu().numpy().reshape(-1)
                C_flatten = C.cpu().numpy().reshape(-1)
                # print(f"X is {X}")
                # print(f"C is {C}")
            except Exception as e:
                print(f"[save_traj] stopping at frame {i} due to: {e}")
                break   # exits the for‐loop, then the function returns

            f.write(f"{t} {X_flatten} {C_flatten}\n")
